...
public class GroovyShellService extends GroovyService {
	...
	public GroovyShellService(Map<String, Object> bindings, int socket) {
		super(bindings);
		this.socket = socket;
	}
	...
	public void launch() {
		...
		try {
			...
			logger.info("GroovyShellService launch() serverSocket: "
					+ serverSocket);
			...
			while (true) {
				...
				try {
					...
					logger.info("GroovyShellService launch() clientSocket: "
							+ clientSocket);
				} catch (IOException e) {
					logger.debug("e: " + e);
					return;
				...
				GroovyShellThread clientThread = new GroovyShellThread(
						clientSocket,
						createBinding());
