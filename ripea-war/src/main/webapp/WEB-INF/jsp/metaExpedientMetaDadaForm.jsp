...
$(document).ready(function() {
	$("#metaDadaId").select2({
		placeholder: "<spring:message code="metaexpedient.metadada.form.placeholder.metadada"/>",
		width: '100%'
	...
	<form:form action="${formAction}" method="post" cssClass="form-horizontal" commandName="metaNodeMetaDadaCommand">
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<rip:inputFixed textKey="metaexpedient.metadada.form.camp.tipus"><spring:message code="metadada.tipus.enum.${metaNodeMetaDada.metaDada.tipus}"/></rip:inputFixed>
			</c:otherwise>
		</c:choose>
		<rip:inputSelect name="multiplicitat" textKey="metaexpedient.metadada.form.camp.multiplicitat" optionItems="${multiplicitatEnumOptions}" optionValueAttribute="value" optionTextKeyAttribute="text"/>
