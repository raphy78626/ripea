...
<c:set var="titol"><spring:message code="contenidor.portafirmes.titol"/></c:set>
<html>
<head>
	<title>${titol}</title>
...
<body>
	<c:if test="${documentVersio.arxiuNom != documentVersio.portafirmesConversioArxiuNom}">
		<div class="alert well-sm alert-warning alert-dismissable">
			...
			<spring:message code="contenidor.portafirmes.conversio.avis"/>
			<a data-rdt-link-modal="true" class="btn btn-xs btn-default pull-right" href="../pdf">
				<spring:message code="contenidor.portafirmes.conversio.boto.previsualitzar"/>
			</a>
		</div>
	</c:if>
	...
	<form:form action="${formAction}" method="post" cssClass="form-horizontal" commandName="portafirmesEnviarCommand" role="form">
		<rip:inputText name="motiu" textKey="contenidor.portafirmes.camp.motiu" required="true"/>
		<rip:inputSelect name="prioritat" textKey="contenidor.portafirmes.camp.prioritat" optionItems="${portafirmesPrioritatEnumOptions}" optionValueAttribute="value" optionTextKeyAttribute="text" required="true"/>
		<rip:inputDate name="dataCaducitat" textKey="contenidor.portafirmes.camp.data.caducitat" required="true"/>
